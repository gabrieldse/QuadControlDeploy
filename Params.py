...
class RLParams:
    def __init__(self):
        ...
        self.max_steps = 8000 #loops throught the PD loop
        ...
        self.q_init = np.array(
            [-0.04, 0.7, -1.4, 0.04, 0.7, -1.4, -0.04, 0.7, -1.4, 0.04, 0.7, -1.4]
        )

        if self.symmetric_pose:
            self.q_init = np.array(
                [-0.04, 0.8, -1.6, 0.04, 0.8, -1.6, -0.04, -0.8, 1.6, 0.04, -0.8, 1.6]
            )

        self.alpha = 0.0  # 3569

        # Other parameters
        self.config_file = "config_go1.yaml"  #  Name of the yaml file containing hardware information
