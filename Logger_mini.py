...
class MiniLogger:
    def __init__(self, size=1e4, dt=0.001):
        ...
        self.filter_torques = np.zeros((size, 12))
        self.command_pos = np.zeros((size, 12))
        self.joint_pos = np.zeros((size, 12))
        self.joint_vel = np.zeros((size, 12))
        self.k = 0
        self.dt = dt

    def append(self, device):
        self.command_torques[self.k] = device.jointTorques
        self.filter_torques[self.k] = device.filterTorques
        self.command_pos[self.k] = device.q_des
        self.joint_pos[self.k] = device.joints.positions.flatten()
        self.joint_vel[self.k] = device.joints.velocities.flatten()
        self.k += 1
