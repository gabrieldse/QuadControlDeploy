...
class pybullet_simulator:
    ...
    def __init__(self, q_init, envID, use_flat_plane, enable_pyb_GUI, dt=0.001):
        ...
        if use_flat_plane:
            ...
            heightPerturbationRange = 0.05 #original:0.05
        ...
        robotStartPos = [0, 0, 10.0]
        robotStartOrientation = pyb.getQuaternionFromEuler([0.0, 0.0, 0.0])  # -np.pi/2

      
        # pyb.setAdditionalSearchPath(
        #    "/home/gabriel/RL_for_Robotics/Solo12/quadruped-rl/urdf/example_robot_data/example-robot-data/robots/go1_description/urdf"
        # )
        # self.robotId = pyb.loadURDF("go1.urdf", robotStartPos, robotStartOrientation)

        pyb.setAdditionalSearchPath(
            "/home/gabriel/RL_for_Robotics/Solo12/quadruped-rl/urdf/example_robot_data/example-robot-data/robots/solo_description/robots"
        )
...
class PyBulletSimulator:
    ...
    def send_command_and_wait_end_of_cycle(self, WaitEndOfCycle=True):
        ...
        self.filterTorques = (
            self._alpha * self.filterTorques + (1 - self._alpha) * self.jointTorques
        )
